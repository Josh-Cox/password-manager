...
namespace PasswordManager.Core.Services
{
    public class CryptoService
    {
        ...
        public byte[] EncryptGcmOld(byte[] plaintext, byte[] key, byte[] nonce)
        {
        ...
        public byte[] DecryptGcm(byte[] encryptedData, byte[] key)
        {
            ...
            if (!plaintext.AsSpan(0, VaultMagic.Length).SequenceEqual(VaultMagic))
                throw new CryptographicException("Invalid vault key");

            return plaintext[VaultMagic.Length..];
        }

        public byte[] DecryptGcmOld(byte[] encryptedData, byte[] key)
        {
            byte[] nonce = new byte[NonceSize];
            byte[] tag = new byte[TagSize];
            byte[] ciphertext = new byte[encryptedData.Length - NonceSize - TagSize];
